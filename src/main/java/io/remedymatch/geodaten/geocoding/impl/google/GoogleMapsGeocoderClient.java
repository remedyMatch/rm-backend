...
public class GoogleMapsGeocoderClient implements Geocoder {
	...
	public String findeAdresseByPoint(@NonNull Point point) {
		...
		return responses.stream().map(geocodingResult -> geocodingResult.formattedAddress).collect(Collectors.toList())
				.iterator().next();
	}
	...
	private List<DirectionsRoute> queryDirectionsService(@NonNull LatLng origin, @NonNull LatLng destination) {
		...
		try {
			...
			final DirectionsResult result = directionsApiRequest.mode(TravelMode.DRIVING).origin(origin)
	...
	private void initApiContext() {
		if (geoApiContext == null) {
			geoApiContext = new GeoApiContext.Builder().apiKey(properties.getGeocoderServiceApiKey()).build();
		}
	}

	@Override
	public double kilometerBerechnen(@NonNull BigDecimal lat1, @NonNull BigDecimal lon1, @NonNull BigDecimal lat2,
			@NonNull BigDecimal lon2) {
