...
public class AngebotJpaRepositoryShould {
	...
	@Transactional
	@Test
	@DisplayName("alle nicht bediente oeffentliche Angebote zurueckliefern")
	void alle_nicht_bediente_oeffentliche_Angebote_zurueckliefern() {
		...
		AngebotEntity nichtOeffentlicheAngebot = angebot(BigDecimal.valueOf(300));
		nichtOeffentlicheAngebot.setOeffentlich(false);
		persist(nichtOeffentlicheAngebot);
		entityManager.flush();
	...
	void alle_nicht_bediente_oeffentliche_Angebote_der_ArtikelVarianteId_zurueckliefern() {
		...
		ArtikelVarianteEntity andereVariante = persist(ArtikelVarianteEntity.builder() //
				...
				.build());
		AngebotEntity andereVarianteAngebot = angebot(BigDecimal.valueOf(400));
		andereVarianteAngebot.setArtikelVariante(andereVariante);
		persist(andereVarianteAngebot);

		entityManager.flush();

		assertEquals(Arrays.asList(ersteAngebot, zweiteAngebot),
				jpaRepository.findAllByDeletedFalseAndBedientFalseAndOeffentlichTrueAndArtikelVariante_Id(
						beispielArtikelVariante.getId()));
	}
	...
	private AngebotEntity angebot(//
		...
		return AngebotEntity.builder() //
				...
				.artikel(beispielArtikel).anzahl(anzahl) //
				.rest(anzahl) //
	...
	private AngebotEntity angebot(final ArtikelVarianteEntity artikelVariante, BigDecimal anzahl) {
		return AngebotEntity.builder() //
				...
				.institution(meinKrankenhaus) //
				.standort(meinStandort) //
				.artikel(beispielArtikel).anzahl(BigDecimal.valueOf(123)) //
				.rest(BigDecimal.valueOf(123)) //
				.steril(true) //
				.haltbarkeit(LocalDateTime.now()).medizinisch(true) //
				.kommentar("Bla bla") //
				.bedient(false) //
				.oeffentlich(true) //
				.build();
