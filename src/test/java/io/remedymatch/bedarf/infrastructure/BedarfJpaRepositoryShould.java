...
public class BedarfJpaRepositoryShould {
    ...
    void artikelKategorieIds_mit_Anzahl_nicht_bedienten_Bedarfe_zurueckliefern() {
        ...
        assertEquals(3, jpaRepository.findAllByDeletedFalseAndBedientFalseAndOeffentlichTrue().size());
    ...
    void artikelIds_mit_Anzahl_nicht_bedienten_Bedarfe_zurueckliefern() {
        ...
        assertEquals(4, jpaRepository.findAllByDeletedFalseAndBedientFalseAndOeffentlichTrue().size());
    ...
    @Transactional
    @Test
    @DisplayName("alle nicht bediente oeffentliche Bedarfe zurueckliefern")
    void alle_nicht_bediente_oeffentliche_Bedarfe_zurueckliefern() {
        ...
        BedarfEntity nichtOeffentlicheBedarf = bedarf(BigDecimal.valueOf(300));
        nichtOeffentlicheBedarf.setOeffentlich(false);
        persist(nichtOeffentlicheBedarf);
        entityManager.flush();

        assertThat(//
                jpaRepository.findAllByDeletedFalseAndBedientFalseAndOeffentlichTrue(), //
    ...
    @Transactional
    @Test
    @DisplayName("alle nicht bediente oeffentliche Bedarfe der ArtikelVariante zurueckliefern")
    void alle_nicht_bediente_oeffentliche_Bedarfe_der_ArtikelVarianteId_zurueckliefern() {
        BedarfEntity ersteBedarf = persist(bedarf(BigDecimal.valueOf(100)));
        BedarfEntity zweiteBedarf = persist(bedarf(BigDecimal.valueOf(200)));
        ...
        ArtikelVarianteEntity andereVariante = persist(ArtikelVarianteEntity.builder() //
                .artikel(beispielArtikel.getId()) //
                .variante("andereVariante") //
                .beschreibung("beschreibung") //
                .build());
        BedarfEntity andereVarianteBedarf = bedarf(BigDecimal.valueOf(400));
        andereVarianteBedarf.setArtikelVariante(andereVariante);
        persist(andereVarianteBedarf);

        entityManager.flush();

        assertThat(//
                jpaRepository.findAllByDeletedFalseAndBedientFalseAndOeffentlichTrueAndArtikelVariante_Id(beispielArtikelVariante.getId()), //
                containsInAnyOrder(ersteBedarf, zweiteBedarf));
    }
