...
public class InstitutionExternalTaskClient {
    ...
    public void doSubscribe() {
        ...
        client.subscribe("institution_antrag_prozess_antrag_ablehen").lockDuration(2000)
                .handler((externalTask, externalTaskService) -> {

                    try {
                        institutionService.antragAblehnen(
                                new InstitutionAntragId(UUID.fromString(externalTask.getBusinessKey())));
        ...
        client.subscribe("institution_antrag_prozess_antrag_genehmigen").lockDuration(2000)
                .handler((externalTask, externalTaskService) -> {

                    try {
                        institutionService.antragGenehmigen(
                                new InstitutionAntragId(UUID.fromString(externalTask.getBusinessKey())));
                        externalTaskService.complete(externalTask);
                    } catch (Exception e) {
                        log.error("Der External Task konnte nicht abgeschlossen werden.", e);
        ...
        client.subscribe("institution_antrag_prozess_inst_anlegen").lockDuration(2000)
                .handler((externalTask, externalTaskService) -> {

                    try {
                        val institution = institutionService.institutionAusAntragAnlegen(
                                new InstitutionAntragId(UUID.fromString(externalTask.getBusinessKey())));
