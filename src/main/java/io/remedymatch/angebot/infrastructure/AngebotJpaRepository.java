...
public interface AngebotJpaRepository extends JpaRepository<AngebotEntity, UUID> {
    ...
    @Query("SELECT new io.remedymatch.angebot.infrastructure.FilterEntry(ak.id as id, count(*) as anzahl) " //
            ...
            + "GROUP BY ak.id")
    List<FilterEntry> findAllKategorienMitUnbedientenAngebotenFilter(@Param("institutionId") UUID institutionId);
    ...
    @Query("SELECT new io.remedymatch.angebot.infrastructure.FilterEntry(ar.id as id, count(*) as anzahl) " //
            ...
            + "WHERE a.institution.id <> :institutionId " //
            + "GROUP BY ar.id")
    List<FilterEntry> findAllArtikelInKategorieMitUnbedientenAngebotenFilter(@Param("kategorieId") UUID kategorieId, @Param("institutionId") UUID institutionId);
    ...
    List<FilterEntry> findAllArtikelVariantenInArtikelMitUnbedientenAngebotenFilter(@Param("artikelId") UUID artikelId, @Param("institutionId") UUID institutionId);
